...
echo Created directory $1/$2. > $1/$2/log.txt
timestamp >> $1/$2/log.txt
echo Started fetching data from $1/$2 repository,
timestamp
echo Started fetching data from $1/$2 repository. >> $1/$2/log.txt 
timestamp >> $1/$2/log.txt
if [ "$5" != "-l" ]
then
...
echo Copied syntaxhighlighter folder to $1/$2 folder. >> $1/$2/log.txt
timestamp >> $1/$2/log.txt
...
do
   ...
   # This completes the noting of comments on a particular issue in our file.
	...
	echo Completed issue number $a. >> $1/$2/log.txt
	timestamp >> $1/$2/log.txt
	a=`expr $a + 1`
done
...
timestamp >> $1/$2/log.txt
echo Completed execution >> $1/$2/log.txt
timestamp >> $1/$2/log.txt
if [ "$4" != "-p" ]
then
rm -r $4/$1/$2
...
mv -f $1/$2 $4/$1
rm -r $1
echo Moved directory $1/$2 to $4/$1/$2. >> $4/$1/$2/log.txt
timestamp >> $4/$1/$2/log.txt
fi
